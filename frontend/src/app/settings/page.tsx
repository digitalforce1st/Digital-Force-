...
import {
  ...
  Brain, Search, MessageCircle, ShieldCheck, QrCode, Radio
} from 'lucide-react'
...
type Tab = 'general' | 'integrations' | 'publishing' | 'autonomous' | 'notifications' | 'danger'
...
export default function SettingsPage() {
  ...
  const [bufferFleetSummary, setBufferFleetSummary] = useState<any>(null)
  const [addingBuffer, setAddingBuffer] = useState(false)
  const [newBufferLabel, setNewBufferLabel] = useState('')
  const [newBufferToken, setNewBufferToken] = useState('')
  const [bufferStatusMsg, setBufferStatusMsg] = useState('')
  const [bufferActionLoading, setBufferActionLoading] = useState<Record<string, boolean>>({})
  ...
  useEffect(() => {
    ...
    fetch(`${BASE}/api/publishing-pool`, { headers: authHeaders() })
      .then(r => r.json())
      .then(data => { if (Array.isArray(data)) setBufferAccounts(data) })
      .catch(() => {})

    fetch(`${BASE}/api/publishing-pool/summary`, { headers: authHeaders() })
      .then(r => r.json())
      .then(data => { if (data) setBufferFleetSummary(data) })
      .catch(() => {})
  ...
  const TABS: { id: Tab; label: string; icon: React.ElementType }[] = [
    ...
    { id: 'publishing', label: 'Publishing Fleet', icon: Radio },
